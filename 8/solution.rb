...
data = DATA.read.each_line.map { |line| line.split(" | ").map { _1.split(" ").map(&:strip).map(&:chars).map(&:to_set) } }.to_h

easy_digits_by_length = {
  1 => 2,
  4 => 4,
  7 => 3,
  8 => 7
...
easy_digit_occurences = data.values.flatten(1).count { easy_digits_by_length.has_value?(_1.count) }
puts "Easy digits (#{easy_digits_by_length.keys.join(", ")}) occured #{easy_digit_occurences} times in outputs"

# Part 2

hard_digits_by_easy_digit_delta = {
  0 => [4, 4, 3, 1],
  2 => [5, 5, 4, 2],
  3 => [3, 3, 2, 2],
  5 => [5, 3, 4, 2],
...
outputs = data.map do |signals, output|
  known_easy = easy_digits_by_length.transform_values do |length|
    signals.detect { _1.length == length }
  end

  known_hard = hard_digits_by_easy_digit_delta.transform_values do |delta|
    signals.detect { |signal| known_easy.values.map { (_1 ^ signal).length } == delta }
  end

  mapping = known_easy.merge(known_hard).invert
  output.map { mapping[_1] }.join("").to_i
end

puts "Sum of outputs is #{outputs.sum}"
